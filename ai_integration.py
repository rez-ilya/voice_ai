        ...
        "parameters": {
            ...
            "properties": {
                "user_command": {
                    "type": "string",
                    "description": (
                        "Английское системное название программы или сайта. "
                        "КРИТИЧЕСКОЕ ПРАВИЛО: Для встроенных приложений Windows используй ТОЧНЫЕ URI-схемы без двоеточия! "
                        "Например: если просят фотографии/галерею -> пиши 'ms-photos', "
                        "если просят календарь -> пиши 'ms-calendar', "
                        "если просят параметры/настройки -> пиши 'ms-settings'. "
                        "Для обычных программ пиши стандартно: 'chrome', 'youtube', 'notepad'."
                    )
                },
                "search_query": {
                    "type": "string",
                    "description": "Поисковый запрос на русском языке, если пользователь просит что-то найти."
                }
            },
...
def ask_ai(question: str) -> str:
    ...
    ACTION_KEYWORDS = [
        ...
        'проверь', 'узнай', 'погода', 'время', 'письма', 'почта', 'вики',
        'найди', 'поищи', 'гугл', 'яндекс', 'расскажи', 'что такое', 'кто такой'
    ]
    is_action_command = any(word in cmd for word in ACTION_KEYWORDS)
    
    if is_action_command:
        system_prompt = (
            "Ты — бортовой ИИ Джарвис, системный переключатель функций.\n"
            "Твоя единственная задача — вызывать инструменты (наборы функций), строго следуя их описанию параметров.\n"
            "Если пользователь просит открыть Галерею, Фото или Календарь, ты ОБЯЗАН передать в параметр user_command "
            "значения 'ms-photos' или 'ms-calendar' соответственно, как указано в инструкции к функции. Никаких просто 'photos'!"
        )
    else:
